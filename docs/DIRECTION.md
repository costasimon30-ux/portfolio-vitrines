...
- **Objectif du portfolio — décision du 18 septembre 2026 :** Simon confirme viser les deux à la fois, sans exclusivité — des clients en freelance et des employeurs potentiels. Le contenu actuel n'est pas modifié par cette décision : le CTA de contact unique et le bloc tarifs « sur devis » (section 4 de la spécification ci-dessus) restent en l'état, rien n'y contredisant une lecture par un recruteur ; le canal de contact déjà en place convient aux deux profils. Aucune nouvelle mention n'est ajoutée sans texte fourni par Simon.
...
#### Statut de l’URL de publication — précision du 19 septembre 2026
...
- Transition de couleur/fond de 120 à 160 ms maximum, sans fondu du texte, rotation d'icône ni translation liée à la bascule. Hors bascule, seules les entrées discrètes par section définies au § 10 sont permises ; elles ne modifient pas la mécanique de la bascule. Préférence de réduction des mouvements : bascule immédiate, sections statiques et défilement non animé. Aucun effet ne conditionne l'accès au contenu.
...
##### 10. Rythme de défilement global discret — direction validée par Simon le 24 septembre 2026

**Portée et statut.** Simon valide pour le seul `sites/portfolio/` une progression visuelle inspirée du GIF fourni : l'effet principal est le défilement naturel qui fait entrer les sections dans le cadre. Cette direction **remplace la limite historique « seule carte Créa’Tif »** du présent § 10 et du § 7 ; elle n'autorise aucun changement de palette, typographie, composition, contenus, CTA, photographie ou destination de lien. C'est une validation de direction créative, non du rendu implémenté ni d'un déploiement.

| Section | Mouvement autorisé à partir de 768 px de largeur CSS effective |
| --- | --- |
| Hero | Entièrement statique dès le premier affichage : nom, accroche, disponibilité et CTA n'attendent aucune entrée. |
| À propos | Titre et bloc de texte entrent **ensemble**, une seule fois, de 10 px depuis la gauche vers leur position ; opacité 0,96 → 1 ; 300 ms `ease-out`, sans décalage entre les paragraphes. |
| Réalisations | Titre et introduction statiques. La carte Créa’Tif **entière** (capture, filet, texte et lien) entre comme un seul bloc, de 8 px depuis la gauche ; opacité 0,94 → 1 ; 280 ms `ease-out`. Elle remplace l'ancien réglage de 200 ms/0,92 sans animer séparément l'image ou le lien. |
| Ce que je fais | Titre et introduction statiques. Cartes de prestations par petits groupes correspondant aux rangées visibles : 8 px depuis le bas, opacité 0,96 → 1 ; 280 ms `ease-out`. Décalage maximal de 60 ms entre cartes d'une même rangée ; pas de cascade sur toute la grille. Le paragraphe technique final reste statique. |
| Contact | Titre, texte et action entrent ensemble de 8 px depuis le bas ; opacité 0,96 → 1 ; 280 ms `ease-out`. Le CTA et l'e-mail restent immédiatement lisibles et activables, sans délai d'accès. |
| Footer et bascule de thème | Statiques ; aucune entrée ni effet supplémentaire. |

**Déclenchement.** Une entrée ne démarre qu'au défilement, lorsque le groupe commence à être substantiellement visible (environ 20 % de sa hauteur dans le viewport). Elle se joue une seule fois, sans replay en remontant, sans délai global ; les 60 ms des prestations sont le seul échelonnement permis. Si un groupe est déjà visible au premier rendu, atteint par une ancre, par le lien d'évitement ou par le focus clavier, il est **immédiatement dans son état final** avant toute lecture ou activation. Ne pas faire attendre une entrée sous prétexte que l'utilisateur est arrivé par saut. Aucun contenu ne part d'une opacité nulle ou ne reste caché en attente d'un scroll.

**Mécanique et replis.** Seuls `opacity` et `transform` peuvent varier ; les éléments conservent exactement leur place et leurs dimensions, sans CLS ni débordement horizontal. Défilement natif : pas de scroll-jacking, parallaxe, déplacement continu corrélé au scroll, sticky forcé, zoom, rotation, rebond, survol animé ou ombre d'élévation. Même trajectoire et même durée dans les thèmes Clair et Sombre. Sous 768 px de largeur CSS effective — y compris quand le zoom replie la page sous ce seuil — toutes les sections sont statiques. Elles le sont également avec `prefers-reduced-motion: reduce`, sans JavaScript ou si l'observation de l'entrée est indisponible. Le thème et les liens restent utilisables indépendamment de la mécanique de mouvement.

**Recette visuelle après implémentation.** Dans les deux thèmes à 320, 375, 768 et 1440 px, puis au zoom navigateur natif 200 % : contenu toujours lisible, jamais tronqué ou caché, aucun décalage de mise en page ni débordement horizontal, trajectoires et durées ci-dessus perceptibles sans dominer le contenu. Tester aussi chargement avec cible déjà visible, arrivée directe sur chaque ancre, lien d'évitement, Tab jusqu'aux liens, retour en arrière, mouvement réduit et JavaScript désactivé : position de lecture, focus et activation doivent rester intacts. À 320/375 px et à toute largeur CSS repliée sous 768 px, constater explicitement l'absence d'animation.

**Critère de rejet.** Si la page ressemble à une démonstration d'effets, si la carte flotte comme un module applicatif, si un texte ou une action attend l'animation, ou si l'ancre, le focus et le scroll sont perturbés, revenir au rendu statique sur le groupe concerné. La validation finale du rendu reste à Simon ; aucune publication n'est autorisée par le présent brief.
