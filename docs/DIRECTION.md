...
- Transition de couleur/fond de 120 à 160 ms maximum, sans fondu du texte, rotation d'icône ni translation liée à la bascule. Hors bascule, seule la révélation ponctuelle de la carte Créa’Tif spécifiée au § 10 est permise ; aucune autre animation d'entrée. Préférence de réduction des mouvements : bascule immédiate et défilement non animé. Aucun effet ne conditionne l'accès au contenu.
...
##### 10. Mouvement ciblé — proposition créative validée par Simon le 24 septembre 2026

**Portée et statut.** Simon valide cette proposition de mouvement pour une évolution ultérieure du seul `sites/portfolio/`. C'est une exception limitée à l'interdiction générale d'animation d'entrée du § 7, sans changement de palette, typographie, composition, thèmes ou contenus. Cette validation porte sur la **direction créative**, pas sur un rendu implémenté ni sur un déploiement ; ceux-ci demanderont leurs contrôles et décisions séparés.

- **Un seul élément animé :** la carte Créa’Tif entière dans « Réalisations » (capture, filet et corps de texte comme un bloc). Le hero, « À propos », les quatre cartes de services, le contact, les CTA, la bascule de thème et le footer restent immobiles. Pas d'effet au survol, d'ombre, de mise en élévation, de parallaxe ou de cartes « flottantes ».
- **Déclenchement et ordre :** aucun mouvement au chargement ni séquence de modules. À partir d'une largeur CSS effective de 768 px, révéler la carte une seule fois lorsqu'environ 20 % de sa surface entre dans le viewport pendant le défilement. Démarrer sans délai ni échelonnement ; ne pas rejouer au retour dans la section. Si la carte est déjà visible au premier affichage ou par arrivée directe sur une ancre, elle reste immédiatement dans son état final.
- **Trajectoire :** translation horizontale de 8 px depuis la gauche vers sa position, opacité de 0,92 à 1 ; durée 200 ms, décélération douce. La carte conserve exactement sa place et ses dimensions dans la grille pendant l'effet. Aucun agrandissement, rotation, rebond, déplacement vertical ou fondu partant d'une opacité nulle. Les textes de la carte et « Voir la démo » ne sont jamais masqués en attendant l'animation.
- **Replis :** sous 768 px, y compris au zoom lorsque la largeur CSS devient inférieure à ce seuil, carte statique. Avec `prefers-reduced-motion: reduce`, sans JavaScript ou si l'observation de l'entrée n'est pas disponible, carte entièrement visible et immobile dès le départ. La navigation au clavier, le lien d'évitement et les ancres ne doivent jamais atteindre une carte dissimulée : si le focus arrive sur « Voir la démo », l'état final est immédiat, sans perte de focus ni de position de lecture.
- **Perception de performance :** ne pas attendre le chargement de la capture ou des polices pour afficher le contenu ; réserver l'espace de la carte afin d'éviter tout décalage. L'effet ne doit pas ralentir le défilement ni provoquer de clignotement ou de retard de lecture. Préférer une observation ponctuelle de l'entrée à un traitement continu du scroll ; seuls opacité et transform sont concernés.

**Critère de réussite :** à 768 et 1440 px dans les deux thèmes, la carte reste d'abord un contenu lisible et accessible ; un observateur attentif perçoit une entrée discrète de 200 ms, sans attendre pour lire ou activer son lien. À 320/375 px, au zoom replié, avec mouvement réduit et sans JS, le rendu est statique et complet.

**Critère de rejet :** abandonner l'effet si la carte semble flotter comme un module applicatif, attire davantage l'attention que la réalisation, si le texte ou le lien disparaît même momentanément, ou si le défilement, l'ancre, le focus ou le chargement subissent un retard visible. Dans ce cas, conserver la carte immobile déjà validée ; ne pas compenser par une animation différente sans nouvel arbitrage de Simon.
