...
##### Évolution photographique ciblée — proposition du 7 septembre 2026

**Statut : proposition soumise à Simon, non validée, non implémentée.** Base consultée après `git pull` : `6b889199b6f751926c17e8828ad9cc1604054a19` (`main`). Sept photos candidates sont présentées ci-dessous ; **aucune image n'est retenue à ce stade**. Les aperçus servent à l'arbitrage et ne sont pas des captures du futur site.
...
###### 2. Emplacements et cadrages à transmettre à Claude après validation
...
| Page / emplacement actuel | Traitement photographique proposé | Desktop | Tablette et petit mobile |
| --- | --- | --- | --- |
| **Accueil — hero**, à la place de la grande composition | **P01 en arrière-plan du hero uniquement**, avec texte et CTA sur un panneau Ivoire opaque à gauche. C'est le principal fond photo du site ; ni la navigation ni toute la page ne deviennent photographiques. | Bandeau d'environ 560–640 px à 1440 px, sans hauteur plein écran ; grille interne 1200 px. Panneau de texte d'environ 440 px, marge intérieure 32 px. Geste visible à droite ; repère focal 70 % / 35 %. La hauteur reste extensible avec le texte. | **Sous 900 px : texte et CTA sur Ivoire, puis photo de contenu 3:2**, sans superposition ni hauteur fixe. Repère 65 % / 40 % ; préserver l'ensemble main–ciseaux–mèche. Le CTA précède l'image. |
...
| **Le salon — bandeau d'ambiance sous l'introduction** | **P04**, sous réserve de validation : remplace la composition architecturale. **Fond photographique du bandeau, sans texte superposé** ; introduction et légende sur Ivoire en dehors de l'image. | **16:9**, repère 50 % / 65 %, garder les deux bacs et la plante. Ne pas recadrer sur la seule brique : elle ne raconte pas le métier. | Photo de contenu **3:2**, pour conserver les bacs. Ne pas forcer un portrait ni remettre les coordonnées au-dessus de l'image. |
...
Ne pas compter sur une ombre de texte ou sur un voile noir « à 40 % » pour garantir le contraste. Aucun texte informatif directement sur les photos n'est prévu dans cette proposition. Si une superposition différente est demandée ensuite, contrôler le contraste sur la zone réellement derrière chaque ligne, dans chaque recadrage ; viser **4,5:1 pour tout texte**, et au moins **3:1 pour les indicateurs visuels nécessaires aux contrôles**. Références : [WCAG — contraste du texte](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html), [contraste non textuel](https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html).

**Photographies d'illustration, jamais preuves commerciales.** Afficher dès l'accueil, sous le hero et en dehors de l'image : « Créa’Tif — concept de salon fictif. Photographies d'illustration. » Pour Le salon, légender P04 : « Ambiance de salon — photographie d'illustration, lieu non associé à Créa’Tif. » Les pages Coiffure et Barbier comportent également une mention d'illustration proche du premier visuel. Les crédits sont accessibles en texte, pas uniquement au survol.
...
###### 6. Arbitrage demandé à Simon et suite limitée

1. **Valider ou refuser l'orientation photographique et les candidats P01–P07**, en particulier le fond P01 de l'accueil et l'atmosphère plus « atelier » de P04. Recommandation : P01/P02/P03 portent les trois entrées métier ; P05/P06/P07 enrichissent les détails sans galerie. P04 n'est à retenir que si son caractère brique/plante correspond au salon imaginé par Simon. Sinon, remplacer uniquement ce candidat par un intérieur clair aux droits vérifiables, sans relancer toute l'identité.
2. **Reporter les choix explicitement approuvés dans cette sélection** avec date et destinations ; conserver « proposée » pour toute image non approuvée. État actuel : **7 proposées, 0 retenue, 0 intégrée**. La vérification des licences ne vaut pas validation artistique ni autorisation de présenter les personnes photographiées comme membres du salon.
3. Après accord, Claude substitue les médias et applique les cadrages spécifiés, sans changer les fonctionnalités ni réécrire les pages. Réserver les proportions avant chargement et fournir des tailles adaptées pour éviter flou, décalages et images inutilement lourdes ; pas d'ajout de dépendance d'animation.
4. La revue aval portera uniquement sur le rendu photographique : métier lisible au premier écran, texte toujours contrasté, gestes non coupés, distinction Coiffure/Barbier, mentions d'illustration et rythme préservés à **375, 768 et 1440 px**, avec contrôle du recadrage à **320 px**. Le hero Coiffure reste horizontal **3:2 entre 600 et 899 px**. Ne pas considérer ces futurs cadrages comme déjà testés ; aucune nouvelle recette fonctionnelle générale n'est engagée par ce brief.
