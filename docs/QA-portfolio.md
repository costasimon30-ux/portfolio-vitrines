...
- Source extraite dans un clone temporaire propre, HEAD détaché exactement à `52584f0d65cb4b1eee7cdcea071794f664b34ae5`. Changement ciblé par ce commit : les deux images WebP, leur notice, `index.html`, `css/style.css` et `publication.json` de `sites/portfolio/`. Les conventions, la direction, l’architecture §§ 5.4–5.6 et les rapports QA/UX et de publication courants ont été relus.
...
- Les deux WebP sont présents et chargent en HTTP local **200 / `image/webp`** : mobile **720 × 480, 28 938 octets**, desktop **896 × 504, 34 280 octets**. Inspection directe : titre, geste de coupe, ciseaux et peigne visibles. La vignette n’étire pas les médias (`object-fit: cover`, cadre 3:2 aux largeurs mobiles et 16:9 à 1440 px), ne déborde pas et ne recouvre pas son texte.
- Chrome local, 100 %, thèmes Clair/Sombre et largeurs `innerWidth` mesurées **320/375/768/1440 px** : huit cas, images chargées après défilement, fichier mobile sélectionné à 320/375/768 et desktop à 1440 ; `scrollWidth = innerWidth`, aucun débordement du cadre ou du texte de la carte. Chrome réglé réellement à **200 %** : les mêmes largeurs initiales donnent `innerWidth` **160/187/384/720 px**, `devicePixelRatio=2` ; huit cas supplémentaires sans débordement ni collision image/texte, y compris le reflow à 768 → 384 px. Les largeurs 320/375 combinent émulation de viewport et zoom natif ; 768/1440 sont des fenêtres aux largeurs indiquées. Les images différées ont été attendues jusqu’à chargement effectif. La capture automatisée au zoom a une incohérence d’échelle ; le constat à 200 % repose sur la géométrie DOM mesurée, pas sur cette capture.
- Dans la carte, « Voir la démo » pointe vers `https://portfolio-vitrines-coiffeur-mixte.costa-simon30.workers.dev/` ; cette adresse répond **200** en HTTPS et présente Créa’Tif. Les mentions « concept de salon fictif », « Photographies d’illustration » et les coordonnées fictives restent présentes/lisibles dans le candidat local ; la démo affiche également le statut fictif. Il ne s’agit pas d’une certification de droits photo ni d’une preuve de version exacte de la démo publiée.
- Non-régression courte, sur le serveur local et à 375 px : premier Tab réel sur accueil et 404 = lien « Aller au contenu principal » avec contour visible ; Entrée place réellement le focus sur `main#main` et ajoute `#main`. Bascule Clair → Sombre fonctionnelle sans perte du focus du bouton. CTA du hero → `/#contact`. Retour de la 404 au clavier → `/`. Sur la matrice vignette : aucune erreur de console/page ni requête en échec/ressource 4xx relevée. Aucun parcours général n’a été relancé.
- Contrat de l’artefact : accueil avec meta `index, follow` et `robots.txt` autorisant l’exploration ; 404 avec meta `noindex, follow` ; `_headers` spécifie `X-Robots-Tag: noindex` et le type texte pour les notices/licences. Ces fichiers et directives sont présents dans le paquet ; leur application par Cloudflare, le statut HTTP réel de la 404 et les MIME du Worker **ne sont pas testables avant publication**. Le serveur de contrôle local ne traite pas `_headers`.
